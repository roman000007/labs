...
class Polynomial :
    ...
    def __mul__(self, rhsPoly):
        newPoly = Polynomial()
        degrees = {}
        currentA = self._polyHead
        while currentA is not None:
            currentB = rhsPoly._polyHead
            while currentB is not None:
                if currentA.degree + currentB.degree not in degrees:
                    degrees[currentA.degree + currentB.degree] = currentA.coefficient * currentB.coefficient
                else:
                    degrees[currentA.degree + currentB.degree] += currentA.coefficient * currentB.coefficient
                currentB = currentB.next
            currentA = currentA.next
        for elem in reversed(sorted(degrees.keys())):
            newPoly._appendTerm(elem, degrees[elem])
        return newPoly
    ...
    def __str__(self):
        current = self._polyHead
        res = ""
        while current is not None:
            if current == self._polyHead:
                res += str(current.coefficient) + "x^" + str(current.degree)
            else:
                res += " + " + str(current.coefficient) + "x^" + str(current.degree)
